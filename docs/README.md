...
| [支付授权篡改与凭证重放 Sandbox 方案](audit/payment-authorization-tamper-sandbox-plan.md) | 四类 sandbox 安全测试方案、授权原则、数据合同、单元测试与实施计划 |
| [Checkout Auto-Fill Target Lock Design](superpowers/specs/2026-05-06-checkout-auto-fill-target-lock-design.md) | 自动填地址目标锁定设计规格（历史设计文档） |
| [三栏工作台 UI 优化设计](superpowers/specs/2026-05-07-three-column-workbench-ui-design.md) | 三栏工作台 UI 设计规格（历史设计文档） |
...
- `docs/superpowers/specs/` 目录下的文档为历史设计规格，用于追溯设计决策，不替代当前 API 参考或架构文档。实际实现以 `main.go` 和 `web/` 为事实来源。
