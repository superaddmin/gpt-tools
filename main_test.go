...
func TestExtractLuckMailVerificationCode(t *testing.T) {
	cases := []struct {
		name string
		body string
		want string
	}{
		{name: "english", body: "Your OpenAI verification code is 123456. It expires soon.", want: "123456"},
		{name: "html", body: "<div>验证码：654321</div>", want: "654321"},
		{name: "plain six digits", body: "Use 778899 to continue", want: "778899"},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := extractLuckMailVerificationCode(tc.body); got != tc.want {
...
func TestSanitizeAuditBodyMasksSensitiveFields(t *testing.T) {
	body := []byte(`{"token":"eyJabc.def.ghi","customer_email":"user@example.com","checkout_session":{"cookie":"secret-cookie"},"otp":"123456","verification_code":"987654","mail_body_html":"<p>secret code</p>","html_body":"<p>html secret</p>","body_text":"text secret","luckmail_token":"luck-secret"}`)
	...
	for _, key := range []string{"verification_code", "mail_body_html", "html_body", "body_text", "luckmail_token"} {
		if detail[key] == "987654" || detail[key] == "<p>secret code</p>" || detail[key] == "<p>html secret</p>" || detail[key] == "text secret" || detail[key] == "luck-secret" {
			t.Fatalf("%s should be masked: %#v", key, detail[key])
		}
	}
...
func TestBuildAuditLogRecordMasksLuckMailSensitiveFieldsWhenEnabled(t *testing.T) {
	oldConfig := config
	config = appConfig{AuditCaptureSensitive: true}
	t.Cleanup(func() { config = oldConfig })

	req := httptest.NewRequest(http.MethodPost, "/api/luckmail/create-and-wait", strings.NewReader(`{"project_code":"openai","luckmail_token":"mail-token-secret"}`))
	responseBody := []byte(`{
		"ok": true,
		...
		"mail_body_html": "<p>123456</p>"
	}`)

	record := buildAuditLogRecord(req, []byte(`{"project_code":"openai","luckmail_token":"mail-token-secret"}`), responseBody, http.StatusOK, time.Now())
	detail, _ := record.OperationDetail.(map[string]any)
	if detail["luckmail_token"] == "mail-token-secret" {
		t.Fatalf("operation_detail.luckmail_token = %#v, want masked", detail["luckmail_token"])
	}
	responsePayload, _ := record.Metadata["response_payload"].(map[string]any)
	if responsePayload["verification_code"] == "123456" {
		t.Fatalf("response_payload.verification_code = %#v, want masked", responsePayload["verification_code"])
	}
	if responsePayload["mail_body_html"] == "<p>123456</p>" {
		t.Fatalf("response_payload.mail_body_html = %#v, want masked", responsePayload["mail_body_html"])
	}
	summary, _ := record.Metadata["response_summary"].(map[string]any)
	if summary["email_address"] != "temp@example.com" {
		t.Fatalf("response_summary.email_address = %#v", summary["email_address"])
	}
	if record.OperationName != "LuckMail 邮箱接码" {
		t.Fatalf("OperationName = %q", record.OperationName)
	}
}

func TestBuildAuditLogRecordMasksLuckMailTokenModeSensitiveFieldsWhenEnabled(t *testing.T) {
	oldConfig := config
	config = appConfig{AuditCaptureSensitive: true}
	t.Cleanup(func() { config = oldConfig })

	req := httptest.NewRequest(http.MethodPost, "/api/luckmail/token-code", strings.NewReader(`{"luckmail_token":"mail-token-secret"}`))
	responseBody := []byte(`{
		"ok": true,
		"stage": "luckmail_token_code_received",
		"email_address": "paid@example.com",
		"verification_code": "654321",
		"html_body": "<p>654321</p>",
		"body_text": "654321"
	}`)

	record := buildAuditLogRecord(req, []byte(`{"luckmail_token":"mail-token-secret"}`), responseBody, http.StatusOK, time.Now())
	detail, _ := record.OperationDetail.(map[string]any)
	if detail["luckmail_token"] == "mail-token-secret" {
		t.Fatalf("operation_detail.luckmail_token = %#v, want masked", detail["luckmail_token"])
	}
	responsePayload, _ := record.Metadata["response_payload"].(map[string]any)
	if responsePayload["verification_code"] == "654321" || responsePayload["html_body"] == "<p>654321</p>" || responsePayload["body_text"] == "654321" {
		t.Fatalf("response_payload contains raw luckmail sensitive fields: %#v", responsePayload)
	}
	if record.OperationName != "LuckMail 已购邮箱验证码" {
		t.Fatalf("OperationName = %q", record.OperationName)
	}
	if record.Metadata["flow_stage"] != "luckmail_purchased_email_code_flow" {
		t.Fatalf("flow_stage = %#v", record.Metadata["flow_stage"])
	}
}
...
func TestLoginClickRecognizesChineseLoginRegisterModalAndUsesTrustedClick(t *testing.T) {
	...
	for _, want := range []string{
		"runLoginWindowProbeStep",
		"runLoginCookieConsentDismissStep",
		"runLoginPageReadinessProbeStep",
		"trusted_login_click_dispatched",
		"cookie_consent_dismissed_before_trusted_click",
		"authSurfaceTextReady",
		"登录或注册",
		"电子邮件地址",
		"使用 google",
		"使用 apple",
		...
		"cookie_banner_interference",
		"homepage_click_handler_unresponsive",
		"route_kind",
		"checkout_page_active",
		"return_homepage_before_login_click",
		"runLoginHomepageRecoveryStep",
		"already_logged_in_session",
		"skip_login_and_continue_session_fetch",
		"logged_in_shell",
		"rebuildManagedLoginBrowserContext",
		"Target.createBrowserContext",
		"Target.disposeBrowserContext",
		"Target.closeTarget",
		"fresh_browser_context_rebuilt",
		...
		if !strings.Contains(script, want) {
			t.Fatalf("login click flow missing %q", want)
		}
	}
...
func TestLoginEmailFillWaitsLongerAfterLoginSurfaceAppears(t *testing.T) {
	source, err := os.ReadFile("main.go")
	if err != nil {
		t.Fatalf("read main.go: %v", err)
	}
	...
	for _, want := range []string{
		"LoginSurfaceReady",
		"login_email_input_not_ready_after_surface",
		"已检测到登录或注册窗口，但邮箱输入框在延长等待后仍未出现",
...
func TestRandomJapaneseProfileUsesUniqueNamesAndAllowedAgeRange(t *testing.T) {
	usedJapaneseProfileNames.mu.Lock()
	previous := usedJapaneseProfileNames.names
	usedJapaneseProfileNames.names = map[string]struct{}{}
	usedJapaneseProfileNames.mu.Unlock()
	defer func() {
		usedJapaneseProfileNames.mu.Lock()
		usedJapaneseProfileNames.names = previous
		usedJapaneseProfileNames.mu.Unlock()
	}()
...
func TestGopayAutoTriggerDecisionRequiresCheckoutGopayAndSubmit(t *testing.T) {
	...
	decision = evaluateGopayAutoTrigger(req, true, false)
	if !decision.Ready {
		t.Fatalf("Ready = false for non-zero due amount, reason = %q", decision.Reason)
	}
	if decision.Conditions["today_due_zero"] != false {
		t.Fatalf("today_due_zero = %#v, want false", decision.Conditions["today_due_zero"])
	}
...
func TestLuckMailTokenModeForcesTokenEmailResolutionAndClearsResolvedEmail(t *testing.T) {
	...
	for _, want := range []string{
		...
		"var emailReady = hasLuckMailToken() ? await ensureLuckMailEmailForGuide()",
		"clearLuckMailResolvedEmailAfterCompletion();",
		"if (!hasLuckMailToken()) return;",
		"luckmailPurchasedEmail.value = \"\";",
...
func TestNormalizePaymentMethodMapsPayPal(t *testing.T) {
	got := normalizePaymentMethod("PayPal")
	if got != "paypal" {
		t.Fatalf("normalizePaymentMethod = %q, want paypal", got)
	}
}

func TestCheckoutPaymentURLForPayPalPrefersOfficialPayPalURL(t *testing.T) {
	checkoutData := map[string]any{
		"url": "https://pay.openai.com/c/pay/cs_live_abc123#fid=test",
	}
	stripeInitBody := map[string]any{
		"next_action": map[string]any{
			"redirect_to_url": map[string]any{
	...
	got := checkoutPaymentURL("paypal", checkoutData, stripeInitBody)
	if got != "https://www.paypal.com/checkoutnow?token=abc123" {
		t.Fatalf("checkoutPaymentURL paypal = %q", got)
	}
}

func TestCheckoutPaymentURLForPayPalRejectsGoPayURL(t *testing.T) {
	got := checkoutPaymentURL("paypal", map[string]any{
		"redirect_url": "https://app.midtrans.com/snap/v4/redirection/acct-123#/payment",
	}, map[string]any{
		"url": "https://app.midtrans.com/snap/v4/redirection/acct-456#/payment",
	})
	if got != "" {
		t.Fatalf("checkoutPaymentURL paypal accepted non-PayPal URL = %q", got)
	}
}

func TestCheckoutPaymentURLForPayPalAcceptsOpenAIHostedCheckoutURL(t *testing.T) {
	paypalCheckoutURL := "https://pay.openai.com/c/pay/cs_live_a13NNQEC6KuKUifTnuM3IEgmh0dt2qd6b3zb1o2nTBsznqFB943f7mVA8N#fidnandhYHdWcXxpYCc%2FJ2FgY2RwaXEnKSdpamZkaWAnPyd%2FbShRUicpJ3ZwZ3Zmd2x1cWxqa1BrbHRwYGtgdnZAa2RnaWBhJz9jZGl2YCknYnBkZmRoamlgU2R3bGRrcSc%2FJ2Zqa3F3amknKSdkdWxOYHwnPyd1blppbHNgWjA0VW82MjJOdmlNV2FnZFViUU9cb1Ftf002YzBhcTRLNHNOMklQdTV0bTV8S1Zkd21jXzNrY2dCMkNDaW09Tkl9U25zYUhSSzBqM0hmMVNhZDNLTWRWa2RTNTVGN1ZnaT1fdicpJ2N3amhWYHdzYHcnP3F3cGApJ2dkZm5id2pwa2FGamlqdyc%2FJyZjY2NjY2MnKSdpZHxqcHFRfHVgJz8ndmxrYmlgWmxxYGgnKSdga2RnaWBVaWRmYG1qaWFgd3YnP3F3cGB4JSUl"
	got := checkoutPaymentURL("paypal", nil, map[string]any{
		"url": paypalCheckoutURL,
	})
	if got != paypalCheckoutURL {
		t.Fatalf("checkoutPaymentURL paypal hosted checkout = %q", got)
	}
}

func TestCheckoutPaymentURLForGoPayKeepsOpenAIHostedURL(t *testing.T) {
	got := checkoutPaymentURL("gopay", nil, map[string]any{
		"url": "https://pay.openai.com/c/pay/cs_live_abc123#fid=test",
	})
	if got != "https://pay.openai.com/c/pay/cs_live_abc123#fid=test" {
		t.Fatalf("checkoutPaymentURL gopay = %q", got)
	}
}

func TestPaypalURLFromStringRejectsPayPalLookalikeHost(t *testing.T) {
	got := paypalURLFromString("https://paypal.com.evil.example/checkoutnow?token=abc123")
	if got != "" {
		t.Fatalf("paypalURLFromString accepted lookalike host = %q", got)
	}
}

func TestStripePaymentMethodFormForPayPalSetsPayPalType(t *testing.T) {
	form := stripePaymentMethodFormForType(nil, "cs_test_123", "pk_test_123", taxRegion{}, "buyer@example.com", stripeClientContext{}, "paypal")
	if got := form.Get("type"); got != "paypal" {
		t.Fatalf("payment method type = %q, want paypal", got)
	}
}

func TestStripeConfirmFormForPayPalSetsExpectedPaymentMethodType(t *testing.T) {
	form := stripeConfirmFormForType(nil, "cs_test_123", "pk_test_123", "pm_123", stripeClientContext{}, "paypal")
	if got := form.Get("expected_payment_method_type"); got != "paypal" {
		t.Fatalf("expected payment method type = %q, want paypal", got)
	}
}

func TestStripeFormsDefaultToGoPay(t *testing.T) {
	paymentMethodForm := stripePaymentMethodForm(nil, "cs_test_123", "pk_test_123", taxRegion{}, "", stripeClientContext{})
	if got := paymentMethodForm.Get("type"); got != "gopay" {
		t.Fatalf("default payment method type = %q, want gopay", got)
	}

	confirmForm := stripeConfirmForm(nil, "cs_test_123", "pk_test_123", "pm_123", stripeClientContext{})
	if got := confirmForm.Get("expected_payment_method_type"); got != "gopay" {
		t.Fatalf("default expected payment method type = %q, want gopay", got)
	}
}

func TestCreatePayPalCheckoutRedirectUsesExistingOfficialPayPalURL(t *testing.T) {
	result := createPayPalCheckoutRedirect(context.Background(), http.DefaultClient, nil, map[string]any{
		"next_action": map[string]any{
			"redirect_to_url": map[string]any{
				"url": "https://www.paypal.com/checkoutnow?token=abc123",
			},
	...
	if !result.OK || result.URL != "https://www.paypal.com/checkoutnow?token=abc123" {
		t.Fatalf("PayPal redirect result = %#v", result)
	}
}

func TestCreatePayPalCheckoutRedirectUsesHostedCheckoutURL(t *testing.T) {
	payURL := "https://pay.openai.com/c/pay/cs_live_abc123#fid=test"
	result := createPayPalCheckoutRedirect(context.Background(), http.DefaultClient, nil, map[string]any{
		"url": payURL,
	}, checkoutRequest{})
	if !result.OK || result.URL != payURL {
		t.Fatalf("PayPal hosted checkout result = %#v", result)
	}
}

func TestCreatePayPalCheckoutRedirectRejectsMissingStripeIdentifiers(t *testing.T) {
	result := createPayPalCheckoutRedirect(context.Background(), http.DefaultClient, nil, map[string]any{
		"url": "https://chatgpt.com/backend-api/payments/checkout",
	}, checkoutRequest{})
	if result.OK || !strings.Contains(result.Error, "missing checkout_session_id") {
		t.Fatalf("PayPal redirect result = %#v", result)
	}
}
...
func TestCheckStripeInitTotalAcceptsPaidSessionAmount(t *testing.T) {
	check := checkStripeInitTotal(map[string]any{
		"total_summary": map[string]any{
...
func TestStripeConfirmFormUsesDynamicExpectedAmount(t *testing.T) {
	form := stripeConfirmFormForType(map[string]any{
		"total_summary": map[string]any{
			"total": float64(34900000),
		},
	}, "cs_test_123", "pk_test_123", "pm_123", stripeClientContext{}, "gopay")
	if got := form.Get("expected_amount"); got != "34900000" {
		t.Fatalf("expected_amount = %q, want 34900000", got)
	}
}
